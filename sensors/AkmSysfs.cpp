...
int writeAkmDelay(int32_t handle, int64_t ns)
{
    ...
    case ID_A:
        ...
        strcpy(delayPath, "/sys/class/compass/akm8975/delay_acc");
        break;
    case ID_M:
        ALOGV("AkmSysfs: Compass: delay=%lld ns", ns);
        strcpy(delayPath, "/sys/class/compass/akm8975/delay_mag");
        break;
    case ID_O:
        ALOGV("AkmSysfs: Orientation: delay=%lld ns", ns);
        strcpy(delayPath, "/sys/class/compass/akm8975/delay_ori");
        break;
    default:
...
int writeAkmEnable(int32_t handle, bool enable)
{
    ...
    case ID_M:
        ...
        strcpy(enablePath, "/sys/class/compass/akm8975/enable_mag");
        break;
    case ID_O:
        ALOGV("AkmSysfs: Orientation: enable=%d", enable);
        strcpy(enablePath, "/sys/class/compass/akm8975/enable_ori");
        break;
    default:
        ALOGE("AkmSysfs: unknown handle %d", handle);
        return -EINVAL;
...
int writeAkmAccel(float x, float y, float z)
{
    ...
    fd = open("/sys/devices/virtual/compass/akm8975/accel", O_WRONLY);
    if (fd < 0) {
